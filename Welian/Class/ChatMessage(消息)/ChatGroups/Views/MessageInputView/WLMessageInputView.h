...
typedef NS_ENUM(NSInteger, WLMessageInputViewStyle) {
    // 分两种,一种是iOS6样式的，一种是iOS7样式的
    WLMessageInputViewStyleQuasiphysical,//ios6样式
    WLMessageInputViewStyleFlat
};

@protocol WLMessageInputViewDelegate <NSObject>
...
@interface WLMessageInputView : UIImageView

@property (nonatomic, weak) id <WLMessageInputViewDelegate> delegate;

//用于输入文本消息的输入框
@property (nonatomic, weak, readonly) WLMessageTextView *inputTextView;

//当前输入工具条的样式
@property (nonatomic, assign) WLMessageInputViewStyle messageInputViewStyle;  // default is XHMessageInputViewStyleFlat
