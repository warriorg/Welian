...
@interface WLMessageInputView ()<UITextViewDelegate>

@property (nonatomic, weak, readwrite) WLMessageTextView *inputTextView;
 ...
 *  @param style 输入框样式类型
 */
- (void)setupMessageInputViewBarWithStyle:(WLMessageInputViewStyle)style ;
...
@implementation WLMessageInputView
...
- (void)setup {
    ...
    _messageInputViewStyle = WLMessageInputViewStyleFlat;
}
...
#pragma mark - layout subViews UI
- (UIButton *)createButtonWithImage:(UIImage *)image HLImage:(UIImage *)hlImage {
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, [WLMessageInputView textViewLineHeight], [WLMessageInputView textViewLineHeight])];
    if (image)
...
- (void)setupMessageInputViewBarWithStyle:(WLMessageInputViewStyle)style {
    ...
    CGFloat height = [WLMessageInputView textViewLineHeight];
    ...
    // 配置不同iOS SDK版本的样式
    switch (style) {
        case WLMessageInputViewStyleQuasiphysical: {
        ...
        case WLMessageInputViewStyleFlat: {
    ...
    if (self.allowsSendVoice) {
        ...
        button = [self createButtonWithImage:WL_STRETCH_IMAGE([UIImage imageNamed:@"VoiceBtn_Black"], edgeInsets) HLImage:WL_STRETCH_IMAGE([UIImage imageNamed:@"VoiceBtn_BlackHL"], edgeInsets)];
...
+ (CGFloat)maxLines {
    return ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPhone) ? 2.0f : 4.0f;
}

+ (CGFloat)maxHeight {
    return ([WLMessageInputView maxLines] + 1.0f) * [WLMessageInputView textViewLineHeight];
}
