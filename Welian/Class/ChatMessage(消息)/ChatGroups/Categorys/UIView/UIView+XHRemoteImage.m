...
- (void)setMessageAvatorType:(WLMessageAvatorType)messageAvatorType {
...
- (WLMessageAvatorType)messageAvatorType {
    return (WLMessageAvatorType)([objc_getAssociatedObject(self, &kXHMessageAvatorTypeKey) integerValue]);
}
...
- (void)loadWithCompletionBlock:(void(^)(UIImage *image, NSURL *url, NSError *error))handler {
    ...
    dispatch_async(self.cachingQueue, ^{
        ...
        if (weakSelf.messageAvatorType != WLMessageAvatorTypeNormal) {
            cacheImage = [WLMessageAvatorFactory avatarImageNamed:cacheImage messageAvatorType:weakSelf.messageAvatorType];
        }
        if (cacheImage) {
...
- (UIImage *)didFinishDownloadWithData:(NSData *)data forURL:(NSURL *)url error:(NSError *)error {
    ...
    if (self.messageAvatorType != WLMessageAvatorTypeNormal) {
        image = [WLMessageAvatorFactory avatarImageNamed:image messageAvatorType:self.messageAvatorType];
    }
    if([url isEqual:self.url]) {
        if(error) {
