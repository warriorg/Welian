...
- (void)viewDidLoad {
    ...
    UIView *headView = [[UIView alloc] initWithFrame:CGRectMake(.0f, .0f, self.view.width, kHeaderWidth + detailHeight + kSegementedControlHeight)];
    ProjectInfoView *projectInfoView = [[ProjectInfoView alloc] initWithFrame:Rect(0, 0, self.view.width, kHeaderWidth)];
    //设置底部边框线
    ...
    //操作栏
    WLSegmentedControl *segementedControl = [[WLSegmentedControl alloc] initWithFrame:Rect(0,projectDetailView.bottom,self.view.width,kSegementedControlHeight) Titles:@[@"项目网址",@"团队成员（23）"] Images:@[[UIImage imageNamed:@"discovery_xiangmu_detail_link"],[UIImage imageNamed:@"discovery_xiangmu_detail_member"]] Bridges:nil isHorizontal:YES];
