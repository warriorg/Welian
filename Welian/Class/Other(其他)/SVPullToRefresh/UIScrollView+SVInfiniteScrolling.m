...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.enabled = YES;
        UILabel *laba = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 150, 44)];
        [laba setCenter:CGPointMake(self.center.x+50, self.center.y)];
        [laba setText:@"加载更多"];
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if(hasCustomView) {
        ...
        CGPoint origin = CGPointMake(roundf((self.bounds.size.width-viewBounds.size.width)/2), roundf((self.bounds.size.height-viewBounds.size.height)/2));
        [self.activityIndicatorView setFrame:CGRectMake(origin.x-100, origin.y, viewBounds.size.width, viewBounds.size.height)];
